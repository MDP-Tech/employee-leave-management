...
exports.employeeLeavedetails = async (req, res) => {
    ...
    if (req.session.employee_id && req.session.role == "hr") {
        if (typeof req_id != "undefined") {
            ...
            sql_query =
                "SELECT e.*,lr.*,concat(e.first_name,' ',e.last_name) as fullname,GROUP_CONCAT(md.document_url SEPARATOR ', ') AS document_urls FROM leave_requests lr,employees e,medical_documents md where e.employee_id=lr.employee_id  and lr.request_id=md.request_id and lr.request_id='" +
                req_id +
...
exports.updateLeaveStatus = async (req, res) => {
    ...
    if (req_id != "") {
        ...
        if (data.mail_status == 'Approved') {
            ...
            carboncopy = 'hemant@shiftwave.com,fathimunnisa@shiftwave.com'
        }else{
        ...
        db.query(sql_query, (err, result) => {
            if (!err) {
                ...
                if (data.leavehistory == undefined) {
                    sql_query1 =
                        "SELECT lr.admin_remark,e.employee_id,lr.request_id,lr.leave_type,lr.Description,lr.start_date,lr.end_date,lr.created_at,lr.status,concat(e.first_name,' ',e.last_name) as fullname FROM leave_requests lr,employees e where e.employee_id=lr.employee_id and e.employee_id='" +
                        req.session.employee_id +
...
exports.employeeDetails = async (req, res) => {
  ...
  FROM permissions p,employees e    WHERE e.employee_id=p.employee_id AND  p.employee_id = ${emp_id}  
   AND p.date BETWEEN DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m-07')
        ...
        res.render("../views/pages/employeeDetails", {
            admin_name: req.session.employee_name,
            result: result,
...
exports.filterDetailsByStatus = async (req, res) => {
    ...
    if (data.leavehistory == 1) {
        ...
        query1 =
            " SELECT lr.employee_id,lr.request_id,CONCAT(e.first_name, ' ', e.last_name) AS fullname,lr.leave_type,DATE_FORMAT(lr.start_date, '%m/%d/%Y') AS start_date,DATE_FORMAT(lr.end_date, '%m/%d/%Y') AS end_date,lr.created_at AS created_at,lr.status FROM leave_requests lr INNER JOIN employees e ON e.employee_id = lr.employee_id WHERE lr.employee_id = '" +
            data.emp_id +
            "' AND DATE_FORMAT(lr.start_date, '%m/%d/%Y') BETWEEN '" +
            formatedstartDate +
            "' AND '" +
            formatedendDate +
            "' AND DATE_FORMAT(lr.end_date, '%m/%d/%Y') BETWEEN '" +
            formatedstartDate +
            "' AND '" +
            formatedendDate +
            "'  ";
        ...
        formatedendDate = endDate[1] + "/" + endDate[0] + "/" + endDate[2];
        query1 =
            " SELECT p.employee_id,p.permission_id,CONCAT(e.first_name, ' ', e.last_name) AS fullname,DATE_FORMAT(p.date, '%m/%d/%Y') AS DATE,p.start_time,p.end_time,p.created_at AS created_at,p.status FROM permissions p INNER JOIN employees e ON e.employee_id = p.employee_id  WHERE  p.employee_id = '" +
            data.emp_id +
            "' AND DATE_FORMAT(p.date,'%m/%d/%Y') BETWEEN '" +
            formatedstartDate +
            "' AND '" +
            formatedendDate +
            "'  ";
...
exports.remainder = async (req, res) => {
    ...
    query =
        " SELECT lr.no_of_days,concat(e.first_name,' ',e.last_name) fullname,e.email,DATE_FORMAT(lr.start_date, '%d/%m/%Y') start_date,DATE_FORMAT(lr.end_date, '%d/%m/%Y') end_date,lr.leave_type,lr.description FROM employees e,leave_requests lr where e.employee_id=lr.employee_id and lr.request_id='" +
        data.req_id +
